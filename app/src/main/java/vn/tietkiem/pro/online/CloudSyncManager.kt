...
class CloudSyncManager(
    ...
    suspend fun restoreSession(): SessionResult? = mutex.withLock {
        ...
        bootstrapLocked(settings.serverUrl, token)
    }
    ...
    suspend fun login(serverUrl: String, email: String, password: String, register: Boolean): SessionResult = mutex.withLock {
        ...
        bootstrapLocked(serverUrl.trim().trimEnd('/'), auth.token, auth.user)
    }
    ...
    private suspend fun bootstrapLocked(baseUrl: String, token: String, knownProfile: CloudApi.UserProfile? = null): SessionResult {
